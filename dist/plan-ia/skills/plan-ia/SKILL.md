...
| 스캔 대상 | 경로 패턴 | 역할 |
|-----------|----------|------|
| REQ | `output/planning/REQ_*.md` | **직접 선행 (매핑 근거)** — FR→페이지 매핑 |
| FN | `output/planning/FN_*.md` | **직접 선행 (매핑 근거)** — FN→페이지 매핑, 복잡도 참고 |
...
|----|----------|------|------|----------|
| V1 | FN 파일 존재 | E | 권장 | output/planning/FN_*.md 존재 확인. 미존재 시 WARN 후 계속 |
