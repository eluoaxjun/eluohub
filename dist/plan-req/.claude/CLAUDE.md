...
1. **자동 연계**: `output/{프로젝트명}/context/qst.md` 존재 → QST 파일도 `output/{프로젝트명}/*/QST_*.md` 패턴으로 읽기
2. **수동 연계**: `input/QST_*.md` 존재 → 해당 파일을 QST 산출물로 읽기 (이전 패키지 산출물 수동 복사 방식)
...
**모드 판정 출력**:
```
[REQ Step 0] 모드: {연계(자동)/연계(수동)/독립} | QST: {n건/없음} | context: {있음/없음} | 참조사이트: {URL/검색결과/미확인}
```
