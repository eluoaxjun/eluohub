...
> `sb-wf-design` 에이전트는 Step 1 완료 후 반드시 수행합니다. wireframe[] 품질 강화를 위해 필수 포함된 에이전트입니다.
...
```bash
# 1. pdf-capture.js 실행 — pdf.js로 페이지별 정확 분할
node .claude/skills/plan-sb/scripts/pdf-capture.js "{PDF경로}" input/
# 결과: input/pdf-page-1.png, pdf-page-2.png, ... + pdf-info.json
```

```
# 2. 페이지별 Read(Vision) — 각 페이지 개별 분석
Read("input/pdf-page-1.png")  → 표지 구조
Read("input/pdf-page-2.png")  → 이력 테이블
Read("input/pdf-page-3.png")  → 본문 레이아웃
...각 페이지를 분석 대상 6개 항목 기준으로 추출
```

**MUST**: pdf-capture.js는 pdf.js로 페이지 단위 정확 분할을 수행한다. 추가 설치 불필요(Playwright 포함).
**MUST NOT**: PDF를 임의 N등분하는 별도 슬라이싱 스크립트 작성 금지. 페이지 경계 오판 원인.
**MUST NOT**: `node -e` 인라인 원라이너로 PDF를 열지 않는다. 반드시 pdf-capture.js 사용.
**MUST NOT**: 사용자에게 PDF 스크린샷을 직접 찍어달라고 요청.

**URL로 PDF 제공 시**: WebFetch로 다운로드 → `input/`에 저장 → pdf-capture.js 실행
...
### Step 0.3: 벤치마킹 (레퍼런스 없을 때 자동 실행)

사용자가 레퍼런스 사이트/이미지/PDF를 제공하지 않은 경우, **자동으로 경쟁사·유사 서비스를 벤치마킹**한다.

**실행 조건**: input/ 폴더에 레퍼런스 파일 0건 + 사용자가 참고 URL 미제공

**벤치마킹 절차**:
1. 프로젝트 업종/서비스 유형 파악 (JSON project 정보 또는 프롬프트에서 추출)
2. WebSearch로 동종 업계 3~5개 사이트 탐색
3. visit.js로 각 사이트 캡쳐 → `output/benchmark/ref/` 에 저장
4. Read(Vision)으로 각 캡쳐 분석:
   - GNB 패턴
   - 레이아웃 구조 (그리드/카드/리스트)
   - CTA 배치
   - 색상/톤 방향
...
**출력 마커**: `[벤치마킹] {업종} — {n}개 사이트 분석 | 핵심 패턴: {요약}`

**레퍼런스 제공된 경우**: 이 단계 스킵. 제공된 레퍼런스로 바로 분석.
...
| 모드 | 조건 | 좌측 패널 (60%) | 설명 |
|------|------|-----------------|------|
| **와이어프레임 모드** (기본) | 신규 구축 / 변경 후 모습을 보여줘야 할 때 | wireframe[] 배열로 자동 렌더링 (generate.js) | 구조적 와이어프레임. PC/MO 동일 방식 |
| **현행 이미지 + 마커 모드** | 운영 수정 / 현행 이미지 위에 변경 지점만 표시할 때 | 현행 캡쳐 이미지 + 빨간 마커 번호 오버레이 | 변경 전 이미지 기준으로 "여기가 바뀝니다" 표시. Description에 수정 전/후 기재 |

**중요 규칙**:
- **현행 캡쳐 = 참고 자료(input/)** — 산출물의 uiImagePath에 현행 캡쳐를 그대로 넣지 않는다 (현행 이미지 모드 제외)
- 와이어프레임 모드에서는 wireframe[] 배열만 사용. 별도 HTML 목업 생성 불필요
- 현행 이미지 모드에서만 uiImagePath에 현행 이미지 사용 + overlay 좌표 지정
- **PC와 MO는 동일 모드를 사용** — PC는 와이어프레임이고 MO는 이미지 같은 혼재 금지

### 와이어프레임 마커 스타일

wireframe[] 내 marker 번호는 generate.js가 자동 렌더링한다. 별도 HTML 목업이나 캡쳐 스크립트 불필요.
...
**필수 구조 최소 요건 (위반 시 Self-Check Fail)**:

| 조건 | 필수 구조 |
|------|----------|
| design 스크린 (페이지) | header 또는 gnb 존재 + group 1개 이상 |
| GNB 요소 | children으로 로고/메뉴/검색 분리. 텍스트 한 줄 나열 금지 |
| 본문 영역 | group으로 감싼 콘텐츠 블록 최소 1개 |
| 카드 2개 이상 | 반드시 group으로 감쌈 (flat 나열 금지) |
| containerType: modal/popup | 위 요건 완화 (group 없어도 허용) |

**MUST NOT**: wireframe에 group 0개 + 요소 5개 이상 = flat 나열. 와이어프레임이 아닌 텍스트 목록이 됨.
...
| **금지** | CSS 속성값 직접 기재 — `#hex`, `px`, `rem`, `margin`, `padding`, `font-size`, `border-radius`, `color:`, `background:` 등. Description은 **기능 의도**만 서술 |

**기능 설계 vs UI 디자인 구분** (SB는 기능 설계 문서):

| 허용 (기능 설계) | 금지 (UI 디자인) |
|----------------|----------------|
| "버튼 클릭 시 목록 갱신" | "버튼 색상 #FF0000, 라운드 8px" |
| "비로그인 시 로그인 유도 팝업" | "폰트 14px 맑은고딕 bold" |
| "검색 결과 0건 시 Empty 상태" | "카드 간격 16px, 그림자 2px" |
| "필터 선택 시 AND 조건 적용" | "그라디언트 배경 적용" |
| "강조 색상으로 표시" (의도) | "#5bb5a2 컬러 적용" (구현값) |
...
| 12 | 정보 소유 경계 준수 | FN 처리 로직·REQ AC 복사 0건 + Description 내 CSS 속성값(#hex, px, rem, margin, padding, font-size, border-radius) 0건 |
...
| 16 | 기획 확정 게이트 | Step 0.5 출력 마커(`[기획 확정]`) 존재 여부. 미존재 시 Fail — 범위 미확정 상태에서 JSON 생성 금지. 독립 모드에서도 최소 변경 범위 확인 필수 |
| 17 | 화면 표현 모드 일관성 | PC/MO가 동일 모드(와이어프레임/현행이미지)인지 확인. 혼재 시 Fail |
| 18 | 현행 이미지 활용 | 와이어프레임 모드에서 현행 캡쳐가 uiImagePath에 직접 사용되면 Fail. 현행 이미지 모드만 허용 |
| 19 | PC/MO 쌍 완전성 | design 스크린이 PC만 존재하고 MO 없으면 WARN. interfaceName 동일인데 viewportType 하나만이면 쌍 불일치 |
| 20 | wireframe 구조 밀도 | design 스크린에 group 0개 + 요소 5개 이상이면 Fail (RF7). containerType modal/popup은 제외 |
| 21 | 복잡 화면 구조 확인 | design 스크린에서 descriptions 5개 이상이면 wireframe[] 구조 충분성 WARN |
...
| RF7 | wireframe에 group 0개 + 요소 5개 이상 | flat 나열 = 텍스트 목록. 와이어프레임 아님 | group으로 영역 묶기 필수 |

## Gotchas (실전 반복 실패 메모)

- **모르면 물어본다**: 사용자 의도 추측 후 진행 → 전체 재작업 유발. 판단이 불확실하면 즉시 질문. "알아서 하다가" 틀린 사례 다수.
