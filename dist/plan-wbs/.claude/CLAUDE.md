...
| 파일 | 존재 시 |
|------|--------|
| `context/fn.md` | 연계 모드 — FN 기능 목록 + 복잡도 기반 공수 산정 |
| `context/ia.md` | 연계 모드 — IA 페이지 정보 추가 참조 |
| `context/req.md` | 연계 모드 — REQ 기능 범위 참조 |
| 없음 | 독립 모드 — 사용자에게 프로젝트 범위·업종·기간 조건 직접 수집 |
...
```
[WBS Step 0] 모드: {연계/독립} | FN: {n건/없음} | IA: {있음/없음} | context: {있음/없음}
→ Step 1 진입
```
