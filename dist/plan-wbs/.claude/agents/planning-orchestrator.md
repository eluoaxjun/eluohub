...
CCD 모드 활성화 시 (`.claude/rules/ccd-autogate.md` 참조), Gate 선택지를 자동 처리한다:

| 조건 | 자동 결정 |
|------|----------|
| Self-Check PASS | → A) 다음 진행 |
| Self-Check FAIL 1~2건 | → 자동 수정 → 재검증 → A) 진행 |
| Self-Check FAIL 3건+ | → 에스컬레이션 |
| DA 전 항목 PM-OK | → A) 진행 (anti-rationalization #2 점검) |
| DA PM-WARN | → A) 진행 (WARN 내용 _context.md 기록) |
| DA PM-BLOCK | → 에스컬레이션 |
| reviewer 90점+ | → A) 진행 |
| reviewer 75~89점 | → 이터레이션 (최대 3회) |
| reviewer 75점 미만 | → 에스컬레이션 |

CCD 모드에서도 **reviewer 실행은 필수**다. 각 스킬 완료 후 Self-Check → reviewer unit 검수를 자동 실행한다.
"B) reviewer 검수"를 사용자 선택지가 아닌 **자동 Gate**로 실행한다.
에스컬레이션 시에만 사용자에게 선택지를 제시한다.
