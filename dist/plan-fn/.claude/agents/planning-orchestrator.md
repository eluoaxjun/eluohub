...
| # | 챌린지 | 질문 | 판단 기준 |
|---|--------|------|----------|
| 1 | **범위 챌린지** | 빠진 기능이 있는가? 리스크는? | 비즈니스 목표 대비 기능 커버리지 확인 |
| 2 | **우선순위 챌린지** | 비즈니스 목표와 정합하는가? | 핵심 기능이 상위에 배치되었는지 |
| 3 | **가정 챌린지** | 확인 없이 추측한 항목이 있는가? | 기능 분해에 근거가 있는지 |
...
```
[PM Router] 모드: {운영/구축} | 프로젝트: {이름} | 기존 산출물: {n건} | ID 시작: {FN-###}
→ plan-fn 호출 준비 완료
```

### Step 2: 기능정의서 생성 (plan-fn)
