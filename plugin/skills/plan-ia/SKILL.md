...
|-----------|----------|-------------------|--------------------|
| REQ | `output/planning/*_REQ_*.md` | FR→페이지 매핑 | - |
| FN | `output/planning/*_FN_*.md` | FN→페이지 매핑 | - |
...
| 스캔 대상 | 경로 패턴 | 역할 |
|-----------|----------|------|
| REQ | `output/planning/*_REQ_*.md` | **직접 선행 (매핑 근거)** — FR→페이지 매핑 |
| FN | `output/planning/*_FN_*.md` | **직접 선행 (매핑 근거)** — FN→페이지 매핑, 복잡도 참고 |
| QST | `output/planning/*_QST_*.md` | 보조 참조 — 업종/타겟/비즈니스 목표 참고 |
| WBS | `output/planning/*_WBS_*.md` | 보조 참조 — 일정 제약 참고 |
| _context | `output/planning/_context.md` | 누적 컨텍스트 — 프로젝트명, 기능 수, 복잡도 분포, 우선순위 |
...
───────────────────────────────────
[생성된 파일]
📄 MD: output/planning/{프로젝트명}_IA_{날짜}_{버전}.md
📄 HTML: output/planning/{프로젝트명}_IA_{날짜}_{버전}.html
───────────────────────────────────
[통계]
총 페이지: {수}개
...
`output/planning/_context.md`가 존재하면 아래 필드를 로드하여 IA 설계에 활용합니다.
...
|----|----------|------|------|----------|
| V1 | FN 파일 존재 | E | 권장 | output/planning/*_FN_*.md 존재 확인. 미존재 시 WARN 후 계속 |
