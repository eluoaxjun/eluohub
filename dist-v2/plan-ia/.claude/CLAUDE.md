...
1. `output/{프로젝트명}/context/` 폴더 스캔
   - `context/fn.md` 존재 → **연계 모드**: FN 파일도 `output/{프로젝트명}/*/FN_*.md` 패턴으로 읽기
   - `context/req.md` 존재 → REQ 파일도 `output/{프로젝트명}/*/REQ_*.md` 패턴으로 읽기
   - 모두 미존재 → **독립 모드**: 사용자에게 프로젝트 기본 정보 수집
2. `output/{프로젝트명}/context/project.md` 읽기 (존재 시)
3. `_context.md` 존재 시 마이그레이션 메시지 출력 후 context/로 전환
4. 산출물 저장 폴더 보장: `output/{프로젝트명}/{YYYYMMDD}/` 없으면 생성

**모드 판정 출력**:
```
[IA Step 0] 모드: {연계/독립} | FN: {n건/없음} | REQ: {n건/없음} | context: {있음/없음}
```
