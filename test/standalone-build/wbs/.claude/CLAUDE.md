...
| 조건 | 방식 | 참조 파일 |
|------|------|----------|
| `output/{프로젝트명}/context/fn.md` 존재 | 자동 연계 | FN 기능 목록 + 복잡도 기반 공수 산정 |
| `output/{프로젝트명}/context/ia.md` 존재 | 자동 연계 | IA 페이지 정보 추가 참조 |
| `output/{프로젝트명}/context/req.md` 존재 | 자동 연계 | REQ 기능 범위 참조 |
| `input/FN_*.md` 존재 | 수동 연계 | 이전 패키지 산출물 수동 복사 방식 |
| `input/IA_*.md` 존재 | 수동 연계 | IA 산출물 수동 복사 방식 |
| 모두 미존재 | 독립 모드 | 사용자에게 프로젝트 범위·업종·기간 조건 직접 수집 |

- `_context.md` 파일이 존재하면 "v1 컨텍스트 파일이 감지되었습니다. `context/` 폴더로 전환합니다."를 출력 후 해당 내용을 참조하고 `context/wbs.md`로 갱신합니다.
- context 파일 없을 시: `mkdir -p output/{프로젝트명}/context/` 생성

```
[WBS Step 0] 모드: {연계(자동)/연계(수동)/독립} | FN: {n건/없음} | IA: {있음/없음} | context: {있음/없음}
→ Step 1 진입
```
